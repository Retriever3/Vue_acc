...
import {BButton, BFormInput, BCard, } from "bootstrap-vue";
import Swal from "sweetalert2";

export default {
  name: "InsertBudget",
  components : {
    BButton, BFormInput, BCard,
  },
  props:
   ["periodNo","workplaceCode","deptCode","accountCode"],
  computed:{
  },
  data(){
    return{
  ...
  methods:{
    ...mapActions('account/budget',['INSERT_BUDGET']),

    clickSave(){
      console.log('등록 클릭 이벤트')
      console.log('deptCode 잘 나오는가',this.deptCode)
      let data={
      ...
      if(Object.values(data).some((dataValue)=>dataValue=="")){
        Swal.fire({
          icon: "warning",
          title: "월별 신청값을 입력해 주십시오",
          customClass:{
            content:'my-font-class'
          },
          showConfirmButton: "확인",
        });
      }else{
        console.log(Object.values(data));
        this.INSERT_BUDGET(data);
        alert('저장되었습니다');
      }
